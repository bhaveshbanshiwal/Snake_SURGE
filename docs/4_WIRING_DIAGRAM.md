...
graph TD
    subgraph Power Source (Mains)
        ...
        PS -->|Red Wire / V+| PHB_V[Serial Bus Servo Driver PHB Terminal: +]
        PS -->|Black Wire / V-| PHB_G[Serial Bus Servo Driver PHB Terminal: -]
    end

    subgraph Computing Brain
        PiPS[Official USB-C Pi Adapter] -->|5V 3A Power| PI[Raspberry Pi 4]
        PI <-->|Data: USB A to Micro USB| Serial Bus Servo Driver[Serial Bus Servo Driver USB Interface]
    end

    subgraph Motor Network (Daisy Chain)
        Serial Bus Servo Driver -->|Plugs Directly On Top| PHB[Serial Bus Servo Driver PHB Power Hub]
        PHB_V --> PHB
        PHB_G --> PHB
        
        PHB <-->|X3P Robot Cable| M1[ST3215 Waveshare Motor 1]
        M1 <-->|X3P Robot Cable| M2[ST3215 Waveshare Motor 2]
        M2 <-.->|X3P Robot Cable| M10[ST3215 Waveshare Motor 10]
    end
...
2. Plug the USB-C end into the **Raspberry Pi 4**.
   - *Safety Note: NEVER try to power the Pi from the Serial Bus Servo Driver or the 10A power supply. Always keep its power isolated via the official adapter to prevent blowing the Pi's fuses.*
...
4. On the **Serial Bus Servo Driver PHB Power Hub Board**, locate the 2-pin screw terminal block.
5. Screw the **V+** wire into the **+** slot on the PHB.
6. Screw the **V-** wire into the **-** slot on the PHB.

### Step 3: The Data Bridge
1. The **Serial Bus Servo Driver USB Interface** is a small stick. It physically mounts onto the pins of the **Serial Bus Servo Driver PHB Power Hub Board**. Press it down firmly so the pins mate.
2. Use a Micro-USB to USB-A cable. Plug the Micro end into the **Serial Bus Servo Driver USB Interface**, and plug the USB-A end into any USB 3.0 (blue) port on the **Raspberry Pi 4**.
   - *This provides the data link between the Pi's Python script and the motors.*

### Step 4: The Daisy Chain (Motors)
1. The ST3215 motors have two identical ports on the back.
2. Take a **Robot Cable X3P**. Plug one end into any 3-pin TTL port on the **Serial Bus Servo Driver PHB Power Hub Board**.
...
Out of the box, all ST3215 Waveshare motors have an ID of `1`. If you daisy-chain them immediately, the code will fail because it cannot distinguish between them.
1. Plug in **only ONE motor** to the Serial Bus Servo Driver PHB.
2. Use the **ST3215 Waveshare Wizard 2.0** software on your Windows PC to change its ID to `1`.
