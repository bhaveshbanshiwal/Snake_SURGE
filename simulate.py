...
# DRAW GROUND MEASUREMENT SYSTEM
for i in range(0, 81): 
    x = i * 0.5 
...
try:
    while p.isConnected():
        ...
        base_pos, base_orn = p.getBasePositionAndOrientation(snakeId) # Head
        tail_state = p.getLinkState(snakeId, num_joints - 1)
        tail_pos = tail_state[0] # Tail
        ...
        # 2. Trace Red path (Head) & Green path (Tail)
        if int(t * 240) % 10 == 0: 
